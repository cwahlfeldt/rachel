  ...
  */
  ...
  plugins: [
    function({ addComponents, config }) {
      ...
      const mediaQueries = Object.keys(screens).map((key) => {
        const width = screens[key]

        return {
          [`@media (min-width: ${width})`]: {
            '.container': {
              'padding-left': key === 'xl' ? config('padding.0') : 0,
              'padding-right': key === 'xl' ? config('padding.0') : 0,
            },
          },
        }
      })
    ...
    function({ addVariant }) {
      return addVariant('important', ({ container }) => {
